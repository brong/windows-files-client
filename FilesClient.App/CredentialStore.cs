...
sealed class CredentialStore
{
    ...
    public record StoredLogin(string LoginId, string Token, string SessionUrl);

    private record CredentialPayload(string Token, string SessionUrl);
    ...
    public void Save(string loginId, string token, string sessionUrl)
    {
        ...
        var payload = JsonSerializer.Serialize(new CredentialPayload(token, sessionUrl));
    ...
    public List<StoredLogin> LoadAll()
    {
        ...
        foreach (var cred in creds)
        {
            try
            {
                ...
                var payload = JsonSerializer.Deserialize<CredentialPayload>(cred.Password);
                if (payload != null)
                {
